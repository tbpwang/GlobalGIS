...
public class Octant implements Polyhedron
{
    ...
    private Octant()
    {
        ...
        // x  is in the east of z, and is perpendicular y and z
    ...
    public Cell getFacet(int ID)
    {
        if (ID < 0 || ID > 7)
        {
            throw new Mistake("ID exceeding");
        }
        Vec4[] vertex = getVertex()[ID];
        LatLon top = Change.getInstance().fromVec4(vertex[0]);
        LatLon left = Change.getInstance().fromVec4(vertex[1]);
        LatLon right = Change.getInstance().fromVec4(vertex[2]);
